...
class EditorClient extends Client {
  ...
  onChange (textOperation, inverse) {
    ...
    // we need to compose them together, so we can undo them one click
    var length = this.editorAdapter.rtcm.codeMirror.doc.listSelections().length
    var multiRangeAttrsChangedSimultaneously = textOperation.baseLength === textOperation.targetLength && length >= 2

    this.undoManager.add(new WrappedOperation(inverse, inverseMeta), compose || multiRangeAttrsChangedSimultaneously)
