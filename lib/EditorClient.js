...
class EditorClient extends Client {
  constructor (data, serverAdapter, editorAdapter) {
    ...
    if (data.clients && Object.keys(data.clients).length) {
      this.clients = Object.values(data.clients).map((client) => {
        return new OtherClient(this.editorAdapter, client.id, client.name, Selection.fromJSON(client.selection))
      })
    }
